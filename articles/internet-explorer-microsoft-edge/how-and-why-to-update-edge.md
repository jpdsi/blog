...
通常 Microsoft Edge (以下 Edge と呼称) のバージョン管理は自動更新によって行われます。

Edge のバージョンで更新が可能であることを端末にインストールされている Edge の更新管理用モジュールが確認すると、そのモジュールはインターネット上から最新の Edge のファイルデータを取得し自動的に Edge をアップデートします。これにより皆様は常に最新版の Edge をご利用いただけます。

ユーザーによっては、ダウンロードによるネットワークの帯域負荷への問題や、ストレージが圧迫されるなどの問題から、アップデートを制御したいというご要望をいただくことがあります。また、勝手にアップデートがされるということ自体がけしからんと思う方もいらっしゃるかと存じます。

こうしたアップデートに関する弊社のデザインとお客様のご要望のミスマッチはなぜ起きるのか。それは弊社の Edge に関するセキュリティへの考え方に理由がございます。
...
- [Edge の種類と Edge のサポート範囲について](#edge-の種類と-edge-のサポート範囲について)
  ...
  - [Edge のバージョン アップデートの頻度について](#edge-のバージョン-アップデートの頻度について)
    ...
    - [最新版の確認方法](#最新版の確認方法)
  - [弊社の Edge のサポート範囲の理由について](#弊社の-edge-のサポート範囲の理由について)
...
- [Edge の更新タイミングの制御方法について](#edge-の更新タイミングの制御方法について)
  ...
  - [MECM/SCCM および WSUS による Edge の更新制御](#mecmsccm-および-wsus-による-edge-の更新制御)
  - [ドメイン管理外の端末での更新制御方法について](#ドメイン管理外の端末での更新制御方法について)

## なぜ Edge は自動更新を行うのか

弊社の Edge を初め Google Chrome などモダンなブラウザは常にセキュリティの脅威にさらされております。大多数のユーザーが利用するアプリケーションの脆弱性を悪用すれば、大多数のユーザーに影響を与えることができます。ブラウザ アプリケーションは絶えずこうしたセキュリティの脆弱性の対応を行い修正したアプリケーションを提供する必要があります。

しかし一方で、ブラウザは莫大なユーザーが利用していることから、個々すべてのユーザーが適切に脆弱性を確認して修正済みの Edge を利用してくれるとは限りません。

例えば、問題が発生する度にユーザーは弊社サイトにアクセスしてダウンロードを行い、インストーラーを起動しなければアップデートされないとするといかがでしょうか。

どのように脆弱性が危険であるかをお伝えしたとしても、全てのユーザーがそうしたアップデートを行ってくださるとは限りません。

そのため、弊社は脆弱性や仕様の不具合などの問題への修正によってユーザーに Edge を管理をする負担をかけないようにするため、修正した Edge アプリケーションをインターネット上に公開し、ユーザーの端末には Edge の更新管理用モジュールを常駐させ、モジュールが新しい Edge を確認したら自動更新するようにデザインをしております。

こちらによりユーザーは面倒くさいインストールをすることなく、問題があれば勝手にブラウザが更新されることで、安全なブラウザ体験をお楽しみいただくことができるようになっております。

こうしたデザインは弊社としても脆弱性の対応やブラウザの不具合の修正などを簡単に行うことができ、開発を効率的に進めることができます。

## Edge の種類と Edge のサポート範囲について

自動更新による常に最新版にアップデートされるというモデルは弊社の Edge のサポートポリシーにも色濃く反映しております。

Edge は以下 5 つのチャネルとしてご提供しております。
- Stable 版
- Extended Stable 版
- Beta 版
- Dev 版
...
通常ユーザーにご利用いただくのは Stable 版であり、Stable 版のバージョンの 1 つ先のバージョンが Beta、その次が Dev、その次が Canary となっております。
...
※ なお全てのドキュメントは英語版のリンクを掲載しておりますが /en-us/ の記述を /ja-jp/ にすることで日本語版のドキュメントを参照可能です。日本語版は自動翻訳のかつアップデートが遅れていることがございます。ドキュメントが英語版のほうが更新が最も早いため最新の情報を確認するためには英語版のドキュメントをご参考いただけますと幸いです。

このうち、**弊社サポートにてお問い合わせを承ることができるのは Stable 版, Extended Stable 版 と Beta 版になります**。

Beta 版、Dev 版、Canary 版などは Stable 版に実装するための機能を先行実験を行い、不具合の修正や機能要望を試すために利用します。

弊社としてサポートする Edge のバージョンは **Stable 版においては "最新版のバージョン - 2" まで**になります。 **Extended Stable 版においては "最新の偶数バージョン - 2"まで**がサポート範囲となっております。
...
Edge のバージョン管理は以下の Chromium と同様であり、AA.BB.CC.DD のバージョンについて、AA は「メジャー バージョン」、BB は「マイナー バージョン」、CC は「ビルド バージョン」、DD は 「パッチ バージョン」となります。
...
Stable 版 のメジャー バージョン アップデートは **約 4 週間** になります。この "約" というところに注意が必要です。あくまでも目安であり多少のズレが発生する場合がございます。
...
あくまでも "約" であり、**Edge のサポート条件は最新版 - 2** までとご理解をいただけますと幸いです。

パッチ バージョン アップデートより細かい頻度で行われますが、こちらは明確な時期は定義しておりません。
...
Extended Stable 版では 最新版の 102 が出ると、次に 106 が出るまではサポート範囲になるため、約 16 週間がサポート範囲になり、Stable 版よりも約 4 週間ほどサポート期限が長くなります。
...
### 弊社の Edge のサポート範囲の理由について

お客様によっては、ブラウザの更新が最長でも 12 週間または 16 週間置きに必要になることに拒絶反応を持たれた方もいらっしゃるかと存じます。

残念ながら現在のインターネットの世界においては、脆弱性や不具合は日々発見されるものであり、そのため対応も日々行わざるを得ません。

従来のウォーター フォール モデルのアプリケーション開発のように、利用するウェブ アプリケーションの動作確認が行えるまでブラウザのアップデートを止め、完動するブラウザ バージョンを明確に定義し、そうして確認したバージョンのみを利用するといったような、ゼロ トレランス ポリシーは通用しえない状況になっております。

さらに、Edge は問題が発見されれば機能修正は次のパッチ バージョンやメジャー バージョンアップデートによって行われます。古いバージョンにおいてとある不具合を発見し、それが最新版の環境においても再現したとしても、その修正は後のメジャー バージョン アップデートやパッチ バージョン アップデートでなければ行われません。

**つまり不具合や脆弱性が確認できたとしても、古いバージョンにおいては絶対に不具合は解消されず、問題解決のためには必然的にアップデートをしなければならないのです**。

よって我々は、動作の確認が行えるまではアップデートを行わないようなゼロ トレランス ポリシーではなく、弊社が事前に提供している Beta 版や、Dev 版、Canary 版などで前もってアプリケーションがご利用できるかを確認いただくことを推奨しております。

**常に最新版によって修正が行われるモダンブラウザである Edge において、古いバージョンをサポートする意味合いが薄れていることから、弊社サポート部門はメジャー バージョンのサポートは約 12 週や約 16 週として設定しております**。

お使いの環境において期待する動作にならない場合は、サポートしている Stable 版 及び Beta 版において事象の発生を確認し、お問い合わせをいただけますと幸いです。

なお、一方で Edge のバージョンが古すぎるためにお問い合わせを受け付けないといったことはございませんのでご安心ください。ただしトラブルシューティングなどを行うに当たって弊社がサポートしているバージョンでの事象の再現や、ログ採取などはサポートしているバージョンにおいて行っていただく必要があるため検証機などのご準備をご依頼する可能性がございます。

## Edge のインストールおよび更新制御方法について

**Edge の更新を制御できるのは、対象がドメイン管理下の端末のみになります**。  
※ ここにおいてのドメイン管理下とは、InTune や Active Directory, Azure Active Directory などの制御下であることを指します。
...
※ MCCM/SCCM や WSUS による制御については後述いたします。  
...
※ よく頂く誤解として Edge のバージョンアップデートは Windows Update で行う方法がわからないというご質問をいただくことがございます。
Edge の更新管理用モジュールと Windows Update とは直接的な関係はございません。 Windows Update を契機として Edge のバージョンが上がるということは基本的にございませんのでご安心いただければと存じます。
...
遅延実行は Edge のインストールがシステム インストールの場合は端末起動後に、ユーザー インストールの場合はログイン後に行われますが、詳細については公開しておりません。

あくまでも UpdatesSuppressed および AutoUpdateCheckPeriodMinutes によって更新タイミングの制御が可能であるとご理解いただけますと幸いです。

※ 上記の遅延実行によって、例えば端末を一斉に同じ時間に起動することによりダウンロードが多重で発生し、ドメイン内のネットワーク帯域が圧迫される可能性はございます。

その場合は端末が一斉に起動するタイミングにおいては UpdatesSuppressed をご設定いただくことで、瞬間的なダウンロードを抑え、それ以降の 1⃣ のチェックは端末によってバラバラですので、負荷をならすことができます。

関連して、Edge の新しいバージョンは段階的なロールアウトによってご提供をしております。すべての端末で一度にアップデート可能な状態になるわけではございません。前提として端末に徐々にアップデートが提供され負荷分散がなされるように配慮しております。
...
**ドメイン管理下外の端末において弊社としてサポートしている更新制御方法はございません**。

本ブログの [なぜ Edge は自動更新を行うのか](#なぜ-edge-は自動更新を行うのか) のチャプターで説明したように、弊社は脆弱性や仕様の不具合などの問題への修正をお客様に負担をかけないようにするために、常に最新版の Edge アプリケーションをインターネット上に公開し、ユーザーの端末には Edge の自動管理用モジュールを常駐させ、モジュールが修正済みの Edge を確認したら自動更新するようにデザインをしております。

そのため **Edge の更新管理用モジュール である MicrosoftEdgeUpdate.exe やそれに類するタスク、サービスなど一切の Edge の更新管理用モジュールに変更を加えることは非サポートとなります**。

そうした変更を加えたことによって Edge の更新がうまく行うことができなくなったというお問い合わせをいただいた場合は、元の状態に戻していただく、または最悪の場合は端末のクリーンインストールをしていただくなどのご提案をさせていただく場合がございます。

また、 **Edge の更新を防ぐために以下の弊社ドキュメントに存在するアップデートにかかわるエンドポイントの通信を止めるといった方法についても弊社としてサポートしておりません**。通信を止めることによってアップデート以外の Edge の機能への不具合を生じる可能性などもございますため、そうして問題が発生したとお問い合わせをいただいた場合はエンドポイントへの通信の許可を再度お願いさせていただく場合がございます。
