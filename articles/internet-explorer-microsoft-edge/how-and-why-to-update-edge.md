...
通常 Microsoft Edge (以下 Edge と呼称) は自動更新によってのバージョンが管理されます。
...
※ MCCM/SCCM や WSUS による制御については後述します。  
...
-	Edge の更新管理用モジュールは、 Edge や WebView2 runtime がインストールされると、通常 1 時間置きに更新チェックのフラグが ON になっているかを確認します。—1⃣
-	フラグが ON の場合は実際にインターネット上に公開されている Edge のサーバーへ更新チェックを行います。—2⃣
...
デフォルトであれば、Edge の更新管理のモジュールは 2⃣ のチェックを行うと、その後 10 時間は 1⃣ のフラグを OFF の状態にします。つまり 2⃣ は最低でも 10 時間ごとにしか行われません。
