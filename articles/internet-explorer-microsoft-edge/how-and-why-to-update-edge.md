...
※ 2023-01-06 追記：  
なお、Edge の更新用モジュールである MicrosoftEdgeUpdate.exe はこれまで UpdateDefault を 0 にしていた場合 Edge ブラウザだけではなく WebView2 の更新も抑止されておりましたが、WebView2 用の GPO があることも鑑み当該動作は不具合であると弊社開発部門で判断し、1.3.171.39 以降は UpdateDefault を 0 に設定していてもWebView2 の更新は抑止されなくなりました。
...
※ 上記の図は正常な端末での動作であり、異常な状態の端末においては上記の図通りの更新チェックが行われない可能性もございます。  
※ 図はインストール後 30 分に更新チェックが起きる想定ですが、実際は Edge をインストール後 0~59 分の間の一意のタイミングです。  
